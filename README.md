...
| `start-worktree` | Worktree-first execution handoff that creates an isolated repo-local `.worktrees/` checkout on a fresh branch, infers repo branch naming when possible, and switches the session into the new worktree before coding begins |
...
- `start-worktree` should be the normal handoff before non-trivial coding so the repository root can remain on the default branch while feature work happens inside `.worktrees/`.
