...
- For Markdown files, prefer drafting first without forcing linting too early. Before committing Markdown work, run `prettier` and then `markdownlint` in a diff-preserving way: prefer new files first, and keep cleanup scoped to touched sections of existing files when practical so the diff stays small. Do not add a default hook for this unless repeated misses prove the manual workflow is insufficient.
